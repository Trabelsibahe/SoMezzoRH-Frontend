...
//Recuperer les taches pour l'RRH
export const GetAllTask = () => {
    return async dispatch => {
      dispatch({ type: taskConstants.GET_ALL_TASK_REQUEST })
      try {
        const res = await axios.get('http://127.0.0.1:3030/api/operation/mytasks')
        if (res.status === 200) {
          dispatch({
...
export const GetAllTaskExpert = () => {
    return async dispatch => {
      dispatch({ type: taskConstants.GET_ALL_TASK_REQUEST })
      try {
        const res = await axios.get('http://127.0.0.1:3030/api/alltasks')
        if (res.status === 200) {
          dispatch({
            type: taskConstants.GET_ALL_TASK,
            payload: res.data,
